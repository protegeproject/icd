...
public class RemoveMetaclass {
    ...
    public static void main(String[] args) {
        if (args.length < 2) {
            System.out.println("Usage: RemoveMetaclass pprj_file_name metaclass_name [top_class_name]+");
            return;
        }
    ...
    private static void addMetaclass(RDFSNamedClass topClass) {
        ...
        for (RDFSNamedClass cls : clses) {
            i++;
            if (i % 500 == 0) {
                Log.getLogger().info("Removed metaclass from: " + i + " classes. Time: " + new Date());
            }
            fixMetacls(cls);
        }
    ...
    private static void fixMetacls(Cls c) {
        if (c.hasDirectType(metaclass) == true) {
            c.removeDirectType(metaclass);
        }
    }
