...
public class WriteWorkerQuestion2 {
	...
	public static void checkMultipleQuestions() {
		try {
			...
			while (cursor.hasNext()) {
				...
				query = new BasicDBObject("type", "doneQuestion");
				...
				while (cursor.hasNext() && i < 2000) {
					...
					DBObject locationObjs = (DBObject) obj
							.get("locationResults");
	...
	public static void writeAnswer(String HITID, String workerID,
			String locString) {
		try {
			Mongo mongo = new Mongo("localhost", 27017);
			DB db = mongo.getDB("mydb");
			...
			BasicDBObject ques = new BasicDBObject("type", "workerAnswer")
					.append("HITID", HITID).append("workerID", workerID)
					.append("locString", locString);
			coll.insert(ques);
			mongo.close();
