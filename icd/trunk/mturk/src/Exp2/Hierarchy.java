...
public class Hierarchy {
	...
	public static void main(String agrs[]) throws Exception {
		...
		// ICDRoot = "Drug-induced hair abnormalities";
		...
		//
		// ICDRoot = "Acquired disorders of the hair shaft";
		// writeDisease(ICDRoot);
		// postQues(ICDRoot, LocationParentChild.get("Body Regions"), false);
		// postQues(ICDRoot, LocationParentChild.get("Body Organ"), false);
		//
		// ICDRoot = "Hirsutism and syndromes with hirsutism";
		// writeDisease(ICDRoot);
		// postQues(ICDRoot, LocationParentChild.get("Body Regions"), false);
		// postQues(ICDRoot, LocationParentChild.get("Body Organ"), false);
		...
		while (itr.hasNext()) {
			...
			if (cur.other > 3) {
				postQues(cur.disease, LocationParentChild.get("Body Regions"),
						false);
				postQues(cur.disease, LocationParentChild.get("Body Organ"),
						false);

				System.out.println("start multiple question : " + cur.disease);
				System.out.println();
			} else {
				processLocationData(cur, ans);//
			}
		}
	}
			...
			while (itrNextDiseases.hasNext()) {
				...
				if (diseaseLocationFinal.get(nextParent).size() != 0) {
					HashSet<String> locations = diseaseLocationFinal
							.get(nextParent);
					locations.add("other");
					if (nextDisease.matches("Hirsutism"))
						System.out.println();
					...
					postQues(nextDisease,
							LocationParentChild.get("Body Organ"), false);
	...
	public static void processLocationData(MultipleQuestion cur,
			HashMap<String, Integer> ans) {

		if (cur.fromParent) {
			cur.locations.remove("other");
	...
	public static void readHierarchyData() throws Exception {
		readICDHierarchy("/Users/Yun/Dropbox/medical/workspace/turk/RICDStrcuture.txt");
